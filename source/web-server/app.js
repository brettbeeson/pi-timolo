...
function show_rel_video(video, days) {
    ...
    let new_video_src = splut['dirname'] + new_video_basename + splut['extension'];
    let h = video.height();
    let w = video.width();
...
function show_rel_image(image, minutes) {
    let current_src = (new URL(image.attr('src'))).pathname;
    let splut = splitPath(current_src);
    let current_image_dt = moment(splut['filename'], DATETIME_FORMAT);
    let new_image_dt = current_image_dt.clone().add(minutes, 'minutes');
    let new_image_basename = FILE_PREFIX + new_image_dt.format(DATETIME_FORMAT);
    // Change date-directory too
    let new_image_dir = splut['dirname'].replace(current_image_dt.format(DATE_FORMAT), new_image_dt.format(DATE_FORMAT));
    let new_image_src = new_image_dir + new_image_basename + splut['extension'];
    let h = image.height();
   let w = image.width();
...
jQuery(document).ready(function () {
        ...
        $(".nav-video-days").click((e) => show_rel_video($("#video-player-div video"), $(e.target).data("days")));
